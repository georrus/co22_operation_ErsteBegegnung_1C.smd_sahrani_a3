...
MissionCondition1 = [ "WIN", "(MVP inArea evac) && (doc inArea evac)", "Объект доставлен. документы приобщены", 15 ];
MissionCondition2 = [ "WIN1", "(!alive MVP || MVP inArea trfinish) && (doc inArea evac)", "Объект отсутствует. документы приобщены.", 20 ];
MissionCondition1 = [ "WIN2", "(MVP inArea evac) && (!alive doc)", "Объект доставлен. документы утеряны", 20 ];
MissionCondition3 = [ "FAIL", "(!alive MVP || MVP inArea trfinish) && (!alive doc)", "Объект отсутствует. документы не приобщены.", 20 ];
MissionCondition4 = [ "WIPED", { {alive _x} count (call BIS_fnc_listPlayers) < 1 }, "All dead", 30 ];
